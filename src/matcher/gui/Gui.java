...
public class Gui extends Application {
	...
	public void start(Stage stage) {
		...
		scene = new Scene(border, 1600, 1000);
		Shortcuts.init(this);
		...
		stage.setTitle("Updater - (https://github.com/kyleescobar)");
		stage.getIcons().add(new Image(Objects.requireNonNull(Gui.class.getResourceAsStream("/app-icon.png"))));
		stage.show();
	...
	public CompletableFuture<Boolean> newProject(ProjectConfig config, boolean showConfigDialog) {
		...
		if (showConfigDialog) {
			...
			dialog.getDialogPane().getStylesheets().add(Config.getTheme().getUrl().toExternalForm());
	...
	public void runProgressTask(String labelText, Consumer<DoubleConsumer> task, Runnable onSuccess, Consumer<Throwable> onError) {
		...
		Scene progressScene = new Scene(pane);
		progressScene.getStylesheets().add(Config.getTheme().getUrl().toExternalForm());
	...
	public Theme getLastSwitchedToTheme() {
		return lastSwitchedToTheme;
	}

	public static File getLastChooserFile() {
		return lastChooserFile;
	}

	private Theme lastSwitchedToTheme;

	private NameType nameType = NameType.MAPPED_PLAIN;
	private BuiltinDecompiler decompiler = BuiltinDecompiler.QUILTFLOWER;
