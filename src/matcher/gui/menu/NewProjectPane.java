...
public class NewProjectPane extends GridPane {
	...
	private void init() {
		...
		add(createFilesSelectionPane("Inputs A", "(New version / unnamed deobs)", pathsA, window, false, false), 0, 0);
		add(createFilesSelectionPane("Inputs B", "(Old version / refactored deobs)", pathsB, window, false, false), 1, 0);
		add(createFilesSelectionPane("Class path A", "(Optional)", classPathA, window, true, false), 0, 1);
		add(createFilesSelectionPane("Class path B", "(Optional)", classPathB, window, true, false), 1, 1);
		...
		add(createFilesSelectionPane("Shared class path", "(Optional)", sharedClassPath, window, true, true), 0, 3, 2, 1);
	...
	private Node createFilesSelectionPane(String name, String hint, ObservableList<Path> entries, Window window, boolean isClassPath, boolean isShared) {
		VBox ret = new VBox(GuiConstants.padding);

		HBox nameCont = new HBox(10);
		nameCont.getChildren().add(new Label(name+":"));

		Label hintLabel = new Label(hint);
		hintLabel.setTextFill(Color.rgb(170, 177, 191));
		nameCont.getChildren().add(hintLabel);

		ret.getChildren().add(nameCont);
